...
import { getAudioUrlCandidates } from './proxy-security'
...
export async function getSongUrl(neteaseId: number): Promise<string | null> {
  const res = await fetch(`/api/qr-login/song-url?id=${neteaseId}`, { cache: 'no-store' })
  ...
  return getAudioUrlCandidates(rawUrl)[0] ?? null
}
